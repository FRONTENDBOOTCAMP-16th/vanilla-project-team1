...
function createSeatButton(i, j) {
  ...
  return { seatElement, seatName };
}

// 스타일 & 접근성 설정
function setSeatTypeUI(seatElement, type, seatName) {
  if (type === RESERVED) {
...
// 그리드(복도)위치 계산
function setSeatGridColumn(seatElement, j) {
  if (j < 2) {
...
function renderSeat(seatArr) {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      // 분리한 함수 호출
      const { seatElement, seatName } = createSeatButton(i, j);
      setSeatTypeUI(seatElement, seatArr[i][j], seatName);
      setSeatGridColumn(seatElement, j);

      // 클릭 이벤트 연결
      attachSeatClickEvent(seatElement);
...
// 클릭 이벤트 로직 리팩토링
function attachSeatClickEvent(seatElement) {
